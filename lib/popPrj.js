...
module.exports = {
	...
	popDeath: function(dataArray, cb) {
		...
		for (i = 0; i < lenareaIds; i++) {
			// for female
			dataDeath.push.apply(dataDeath, genderDeath('female', areaIds[i]));
			
			// for male
			dataDeath.push.apply(dataDeath, genderDeath('male', areaIds[i]));
		}
	...
	popCombine: function(data1YearAdd, data0Age, dataDeath, cb) {
		...
		var getAllAges = function (dataAll, cb) {
			...
			for (i = 0; i <lenareaIds; i++) {
				// for female
				dataAll.push.apply(dataAll, genderAllAges('female', year, areaIds[i]));

				// for male
				dataAll.push.apply(dataAll, genderAllAges('male', year, areaIds[i]));
			}
