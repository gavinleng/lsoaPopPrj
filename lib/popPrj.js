...
module.exports = {
	...
	popDeath: function(dataArray, cb) {
		var i, j, k, lenratDeath, ratDeathF, ratDeathM, dataAllAges, dataAges, lendataAges;
		...
		for (i = 0; i < lenareaIds; i++) {
			// for female
			dataAllAges =  _.filter(_dataArray, {'area_id':areaIds[i], 'gender':'female', 'year':dataArray[0].year});
			
			ratDeathF = _.filter(ratDeath, {'gender':'female'});
			lenratDeath = ratDeathF.length;
			
			for (j = 0; j < lenratDeath; j++) {
				if (ratDeathF[j].age_band != '90+') {
					dataAges = _.filter(dataAllAges, function (item) {
						return ((+item.age_band >= +ratDeathF[j].age_band.split('-')[0]) && (+item.age_band <= +ratDeathF[j].age_band.split('-')[1]) && (item.gender == 'female'));
					});
					...
					for (k = 0; k < lendataAges; k++) {
						...
						dataAges[k].persons = +(+dataAges[k].persons * (+ratDeathF[j].rate) / 1000).toFixed(2);
						
						dataDeath.push(dataAges[k]);
					}
				} else {
					dataAges = _.filter(dataAllAges, {'age_band':'90+'});
					
					dataAges[0].year =(+dataAges[0].year + 1) + '';
					dataAges[0].persons = +(+dataAges[0].persons * (+ratDeathF[j].rate) / 1000).toFixed(2);
					
					dataDeath.push(dataAges[0]);
				}
			}
			
			// for male
			dataAllAges =  _.filter(_dataArray, {'area_id':areaIds[i], 'gender':'male', 'year':dataArray[0].year});
			
			ratDeathM = _.filter(ratDeath, {'gender':'male'});
			lenratDeath = ratDeathM.length;
			
			for (j = 0; j < lenratDeath; j++) {
				if (ratDeathM[j].age_band != '90+') {
					dataAges = _.filter(dataAllAges, function (item) {
						return ((+item.age_band >= +ratDeathM[j].age_band.split('-')[0]) && (+item.age_band <= +ratDeathM[j].age_band.split('-')[1]) && (item.gender == 'male'));
					});
					...
					for (k = 0; k < lendataAges; k++) {
						dataAges[k].year = (+dataAges[k].year + 1) + '';
						dataAges[k].persons = +(+dataAges[k].persons * (+ratDeathM[j].rate) / 1000).toFixed(2);
					...
					dataAges[0].year =(+dataAges[0].year + 1) + '';
					dataAges[0].persons = +(+dataAges[0].persons * (+ratDeathM[j].rate) / 1000).toFixed(2);
