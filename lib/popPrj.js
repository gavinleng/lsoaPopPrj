...
module.exports = {
	...
	popDeath: function(data1YearAdd, data0Age, cb) {
		var i;
		
		var dataArray = data1YearAdd.concat(data0Age);
		...
		var genderDeath = function (gender, areaIdsg) {
			...
			for (j = 0; j < lenratDeath; j++) {
				if (ratDeathg[j].age_band != '90+') {
					...
					for (k = 0; k < lendataAges; k++) {
						dataAges[k].year = dataAges[k].year;
					...
					dataAges[0].year = dataAges[0].year;
